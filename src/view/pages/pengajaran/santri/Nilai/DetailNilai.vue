        ...
        <v-tooltip top>
            <template v-slot:activator="{ on, attrs }">
                <v-btn
                    ...
                    v-on="on"
                    @click="editItem(item)"
                    v-show="(accessList.U && item.tugas !== null && item.tugas !== '') || (data_akses === 1 && item.tugas !== null && item.tugas !== '')"
                >
        ...
        <v-tooltip top>
            <template v-slot:activator="{ on, attrs }">
                <v-btn
                    ...
                    v-show="(accessList.D && item.tugas !== null && item.tugas !== '') || (data_akses === 1 && item.tugas !== null && item.tugas !== '')"
                >
                    <v-icon dark>
                        mdi-delete
                    </v-icon>
...
export default {
    ...
    methods:{
        ...
        getMasterRiwayatNilai(idHeader){
            ...
            this.$store
                .dispatch(Fetch_R_Nilai_Inti, { SantriId, TahunId })
                .then((res) => {
                    this.CustomMessage = res.message_opt
                    this.data_nilai = res.data
                })
                .catch((err) => err);
        },
